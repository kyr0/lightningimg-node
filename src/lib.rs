...
fn convert_image(input_path: &Path, output_dir: Option<&Path>, keep_original_ext: bool) -> Result<()> {
    let output_path = if keep_original_ext {
        ...
        output_dir
            .map(|dir| dir.join(input_path.file_stem().unwrap()).with_extension("webp"))
...
#[napi]
pub fn process_directory_destructive(input_dir: String, keep_original_names: Option<bool>) -> Result<()> {
...
fn process_directory_internal(input_dir: &str, output_dir: Option<&str>, keep_original_ext: bool) -> Result<()> {
...
fn is_supported(path: &Path) -> bool {
    matches!(path.extension().and_then(|ext| ext.to_str()), Some("jpg" | "jpeg" | "png" | "bmp" | "tiff"))
}